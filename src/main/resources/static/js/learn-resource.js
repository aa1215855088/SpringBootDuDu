...
$(function () {
    ...
    $(grid_selector).jqGrid({
        ...
        colModel: [
            {label: '图书ID', name: 'bookId', width: 75},
            {label: '图书名称', name: 'bookName', width: 150},
            {label: '图书价格', name: 'bookPrice', width: 150},
            {label: "图书作者", name: 'bookAuthor', width: 150}
        ],
    ...
    $('#saveLearnBtn').click(function () {
        $('#bookFrom').data('bootstrapValidator').validate();
        if (!$('#bookFrom').data('bootstrapValidator').isValid()) {
            return
        } else {
            saveLearn();
        }
...
function refreshData() {
    $(grid_selector).jqGrid('setGridParam', {
        ...
        page: $(grid_selector).getGridParam('page')
    }).trigger("reloadGrid");
}
